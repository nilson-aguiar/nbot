...
class PayeeMappingRepositoryAdapter(
    ...
    override fun findAll(): List<PayeeMapping> = jpaRepository.findAll().map { it.toDomain() }

    override fun findById(id: UUID): PayeeMapping? = jpaRepository.findById(id).map { it.toDomain() }.orElse(null)
