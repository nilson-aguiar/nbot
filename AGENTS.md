...
12. **Import Preference**: Never use fully qualified class names anywhere in the code (methods, variables, types, etc.); always use imports to keep the code clean and idiomatic. To make code context-rich and avoid ambiguity, using **import aliases** is encouraged (e.g., `import org.telegram...File as TelegramFile`), especially when dealing with classes that have similar names across different packages.
